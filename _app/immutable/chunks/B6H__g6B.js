import{a as E,f as z}from"./B3KKMlPP.js";import"./C9Vuyvrt.js";import{am as s,an as x,ao as n,al as A}from"./BQfckOSY.js";import{h as a}from"./CjXTrQwK.js";const W={title:"Beating memcmp",date:"2021-08-05T00:00:00.000Z",excerpt:"A real struggle for comparison",description:"A software developer's attempts to outperform memcmp.",front:!0},{title:O,date:M,excerpt:C,description:D,front:G}=W;var B=z('<h2>A Quick Brief</h2> <p>Under usual conditions this is something no one really should need, want or have to do. <code>memcmp</code> after all is one of the most obvious things to optimize in any program as it’s essentially a generalization of one of the most basic instructions <code>cmp</code>. It’s also seriously unlikely we’ll outperform memcmp on our own.</p> <p>Lets first go over a few optimizations that are likely happening when you make use of memcmp. Provided we’re using a compiler that analyzes code, it might be inclined to notice when memcmp is used with a fixed length.</p> <pre class="language-c"></pre> <p>Here the compiler might recognize that a 4 byte wide comparison is in fact, just a <code>cmp</code> instruction, better yet it might save on wome work by just using <code>sub</code> and sticking the resulting value into the return register.</p> <p>Suprisingly this is roughly most of what makes a modern memcmp quick, even with a non-fixed size a quick and dirty loop 4 bytes at a time is absurdly fast. It sets flags as well so we can follow it immediately with a conditional jump to exit asap.</p> <p>A bit of an implicit optimization here with a fixed length comparison is that we don’t need to loop, provided a reasonable comparison length, the compiler might remove all loops and just spit out a series of <code>sub</code> instructions or equivalents.</p> <p>Another possibility is it might make the comparisons branchless…although we might note here if the compiler is optimizing heavily enough it might rewrite our memcmp to branch only once to exit the loop to fit our needs. To get an idea, lets picture what our own <code>memcmp</code> might look like.</p> <pre class="language-c"></pre> <pre class="language-c"></pre> <p>Here a branchy dissasembly you could propagate the <code>== 0</code> condition to the loops, jumping when <code>!= 0</code>.</p> <p>Of course this is simplified from what you might spot inside <code>memcmp</code> disassembly, but lets see how it does.</p> <pre class="language-undefined"></pre> <canvas class="bar_chart"></canvas> <p>Huh, wait, what? We already beat memcmp? What happened?</p> <p>Well obviously this doesn’t provide full context, here we’re testing 17 bytes. Meaning we’re dealing with two loops, one which is blazing fast, and one which chews through only 1-3 bytes. Worse yet these benchmarks are somewhat long (around 12 seconds) and warming the cache. Maybe we wouldn’t do so well if we jumped right into the code cold.</p> <p>So happens when the cache is dirty does our simple version still do ok?</p> <pre class="language-undefined"></pre> <canvas class="bar_chart"></canvas> <p>Wooo… we’re riding the edge, removing the minimum run length from the benchmarks means roughly that this is benchmarking one execution each, not a lot of wiggle room to warm a cache. Obviously this isn’t exactly a reliable test, a cold cache shouldn’t do as well as a warm one after all. But we’ll skip over that for now. What happens with different compilers is this even portable? So far these results are from clang, what happens if we try MSVC?</p> <pre class="language-undefined"></pre> <canvas class="bar_chart"></canvas> <canvas class="bar_chart"></canvas> <p>Here I’ve added some additional tests for 64bit, and we start to see a bit of variation. In fact here we can see that with as quick a run as possible without hitting the cache hard we’re dancing around <code>memcmp</code>. Also, we can see the results are at least an order of magnitude of each other, so at least we’ve not shot the pooch at the start.</p> <p>Here’s what our byte_cmp (native) looks like.</p> <pre class="language-c"></pre> <canvas></canvas> <p>Couple interesting things to note, one our 64 bit version is likely tied with our 32 bit approach as well as memcmp. However now we’re not feeling nutty, surely this function wouldn’t outdo memcmp right off the bat!</p> <p>And unfortunately if you benchmark a bit, it won’t be too long before something like this happens (even in clang):</p> <pre class="language-undefined"></pre> <canvas class="bar_chart"></canvas> <p>But we’re certainly close. Well…</p> <h3>Hold up</h3> <p>Why? Because all of this so far is a prime example of why we need to be careful of optimizations when benchmarking.</p> <p>What happened? Both clang and MSVC could see though the test functions I wrote and performed another optimization. Here is what my test looked like:</p> <pre class="language-c"></pre> <p>There’s an obvious comparison which is trivially true, but the compiler also works out that memcmping the same pointer is going to always return true. That’s right, so far all these results are bunk, why? Because it’s entirely dependant on how optimized a simple move is (which can also be optimized away into just an increment). Doh…</p> <p>Ok lets…try this again, this time we’ll rewrite our test to use two different std::strings to store some memory buffers, and lets randomize their contents (once, the same inputs for all tests*).</p> <pre class="language-undefined"></pre> <canvas class="bar_chart"></canvas> <p>Ok, lesson learned, need to be careful how we write our tests.</p> <p>Wait what? We beat memcmp again? Did we get lucky?</p> <pre class="language-undefined"></pre> <canvas class="bar_chart"></canvas> <p>Seems like the answer is we beat it handily, and an almost factor of two is nothing to scoff at. But hey, at least we’ve learned something, if our compilers pick up on it, they can very easily double our throughput if we do optimize our memcmps away. So where do we go from here? We beat memcmp…can we do better?</p> <p>(I’m going to give away here that past this point, this is about the best I’ve done as of writing this)</p> <p>There are other things we can try, namely for example the code we’re generating with the code above is branchy, we could attempt to force the compiler to generate non-branchy code and see what happens.</p> <pre class="language-c"></pre> <p>Here we’re accumulating the result into the end buffer…and we’re somewhat forgoing how memcmp functions, but that should be ok, after all I’m at least not interested necessarily which buffer looks greater or lesser than another buffer. I really only care if they’re equal or not.</p> <pre class="language-undefined"></pre> <canvas class="bar_chart"></canvas> <p>I secretly ran this test with the other bunch…and…it’s not looking good, in fact we’ve managed to seriously hamper our throughput, so badly in fact we fell way bellow memcmp, bummer. What if we try writing it a different way? Maybe other instructions will do better?</p> <pre class="language-c"></pre> <pre class="language-undefined"></pre> <canvas class="bar_chart"></canvas> <p>And here we can see again the swings in performance between each run are a bit of a pain to work with, here we manage to beat memcmp…but we’re nowhere near our best, memcmp reliably sits around 3.6 GB/s and our not too shabby byte cmp is staying almost at the 8 GB/s range.</p> <p>But again, this is a little odd, all things considered, we shouldn’t beat memcmp so easily. So here’s the final kicker, our tests, still not great. Randomized inputs are fine and all, but decently random inputs aren’t likely to repeat, meaning what this mostly shows is the speed up relative to failing the first branch. Here we can see our simple loops net us a big win when branching and exiting the loop almost immediately. Any extra work memcmp might be doing at the start is where we’re winning out…this also is likely why our branchless approach were so penalized, for every exeuction we were processing a larger buffer.</p> <p>Lets just eyeball this sizable list of tests to get a sense for what just happened.</p> <pre class="language-undefined"></pre> <p>And now lets apply our “fix”. As we increase the buffer we’re dealing with we also increase the number of characters that are shared between each buffer. This will give us a rough sense of what the throughput is like when the buffers are actually similar.</p> <pre class="language-undefined"></pre> <canvas class="bar_chart"></canvas> <p>And now we can see that our branchless optimization was in fact an optimization…although it really needed to be operating on larger buffer. These tests I added an extra random 16 bytes at the end (so they’re still not likely to ever match), but now we can see the throughput. At this point I might be tempted tune this function and really put the squeeze on memcmp in all cases.</p> <p>But I know one last thing, which is the dissassembly I was trying to mock up with my example. If we take a peek at memcmp’s you’ll be finding that memcmp is also tuned a bit. In MSVC at least memcmp is checking the size of the buffer and doing something different when the buffer is large. clang I have to assume is doing something similar. In short in order to beat memcmp we’ll have to absorb the cost of an additional branch and then optimize two different cases (one for a larger vs smaller buffer). This also explains why we won out with our quick and dirty example when the data was completely random, we were avoiding the cost of a branch instruction / misprediction on the byte’s size. We can also see here how small a window we had, a single branch misprediction was likely the entire difference, and it only held out for maybe 73 - 105 matching bytes.</p> <p>Obviously though this probably invokes a few bits of advice we’ve all likely heard over and over, don’t reinvent the wheel, test and benchmark your code, know your hardware (I’m working with an i7 6800k (x64)) and probably a lot more etc etc. I would say though at least at an individual level don’t just trust your library writers to write general code to fit a specific use case. Here for example I know what I was after (I definitely would’ve liked to have definitively destroyed memcmp in a benchmark with something so simple), but I also know my inputs. Although my example wasn’t a decisive winner (and it definitely fell off with larger inputs) I know that it would be for what I’m dealing with.</p> <p>But is this truely the best we can do? Are there no tricks left? Well of course not, as we can see here we’ve learned a key detail, one we’re paying a hefty price for going branchless…but so far we’ve gone all in. A branch after all is a chance to exit from the loop, this means that a branchless approach heavily favors processing inputs which match, where a branchy approach heavily favors ones which don’t. Obviously it entirely depends on what you’re dealing with, but a fair guess would lean that of all the input combinations, two inputs which match are less likely than those which don’t. Suppose we compromise? Do chunks of data branchlessly and then branch less often?</p> <pre class="language-c"></pre> <pre class="language-undefined"></pre> <canvas class="bar_chart"></canvas> <p>Ok now we’re definitely beating memcmp…what else can we do? How about sse?</p> <pre class="language-c"></pre> <pre class="language-undefined"></pre> <canvas class="bar_chart"></canvas> <p>Ok…also not bad, but not exactly much better than what we had.</p> <p>I’m going to stop here since I’m happy with my results, granted there’s another consideration for <code>memcmp</code>, suppose we know one buffer is a constant? Can we get c++ to generate an unrolled loop of comparisons? Will that beat these tight loops? What if we force the compiler to vectorize the loops?</p> <p>I should add, if you’re staring at <code>byte eq</code> and <code>byte eq (!=)</code> that <code>byte eq (!=)</code> does not become branchless. That’s right, you do need to be careful of which operators you’re using when trying to branchless code. In the <code>byte eq (!=)</code> case it’s actually roughly equivalent to how byte cmp functions. Roughly speaking your best bet is to stick to bitwise operations, logical ones like <code>!=</code> the compiler will consider the branchy approach to be the faster one.</p> <p>Forgive the data dump, I’ve yet to find a nice way to plot this out in html.</p>',1);function R(U){var v=B(),t=s(x(v),6);a(t,()=>'<code class="language-c"><span class="token keyword">int</span> result <span class="token operator">=</span> <span class="token function">memcmp</span><span class="token punctuation">(</span>buf_ptr0<span class="token punctuation">,</span> buf_ptr1<span class="token punctuation">,</span> <span class="token number">4</span><span class="token punctuation">)</span><span class="token punctuation">;</span></code>',!0),n(t);var e=s(t,10);a(e,()=>`<code class="language-c">constexpr <span class="token keyword">int</span> <span class="token function">bytecmp</span><span class="token punctuation">(</span><span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> buf_ptr0<span class="token punctuation">,</span> <span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> buf_ptr1<span class="token punctuation">,</span> <span class="token class-name">size_t</span> bytes<span class="token punctuation">)</span> noexcept <span class="token punctuation">&#123;</span>

    <span class="token keyword">int</span> ret <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span>

    <span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_count <span class="token operator">=</span> bytes <span class="token operator">/</span> <span class="token keyword">sizeof</span><span class="token punctuation">(</span><span class="token keyword">int</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_rem <span class="token operator">=</span> bytes <span class="token operator">%</span> <span class="token keyword">sizeof</span><span class="token punctuation">(</span><span class="token keyword">int</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token comment">//loop 4 bytes at a time</span>
    <span class="token keyword">for</span> <span class="token punctuation">(</span><span class="token class-name">size_t</span> i <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span> i <span class="token operator">&lt;</span> loop_count<span class="token punctuation">;</span> i<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
        <span class="token keyword">int</span> tmp <span class="token operator">=</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> <span class="token keyword">int</span><span class="token operator">*</span><span class="token punctuation">)</span>buf_ptr0<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span> <span class="token operator">-</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> <span class="token keyword">int</span><span class="token operator">*</span><span class="token punctuation">)</span>buf_ptr1<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span><span class="token punctuation">;</span>
        <span class="token keyword">if</span> <span class="token punctuation">(</span>tmp <span class="token operator">!=</span> <span class="token number">0</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
            <span class="token keyword">return</span> ret <span class="token operator">=</span> tmp<span class="token punctuation">;</span>
        <span class="token punctuation">&#125;</span>
    <span class="token punctuation">&#125;</span>
    <span class="token comment">//bump forward</span>
    buf_ptr0 <span class="token operator">+=</span> loop_count <span class="token operator">*</span> <span class="token keyword">sizeof</span><span class="token punctuation">(</span><span class="token keyword">int</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    buf_ptr1 <span class="token operator">+=</span> loop_count <span class="token operator">*</span> <span class="token keyword">sizeof</span><span class="token punctuation">(</span><span class="token keyword">int</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token comment">//loop a byte at a time</span>
    <span class="token keyword">for</span> <span class="token punctuation">(</span><span class="token class-name">size_t</span> i <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span> i <span class="token operator">&lt;</span> loop_rem<span class="token punctuation">;</span> i<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`,!0),n(e);var o=s(e,2);a(o,()=>'<code class="language-c">bool result <span class="token operator">=</span> <span class="token punctuation">(</span>test<span class="token punctuation">.</span>size <span class="token operator">>=</span> test<span class="token punctuation">.</span>size<span class="token punctuation">)</span> <span class="token operator">&amp;&amp;</span> <span class="token function">bytecmp</span><span class="token punctuation">(</span>test<span class="token punctuation">.</span>ptr<span class="token punctuation">,</span> test<span class="token punctuation">.</span>ptr<span class="token punctuation">,</span> test<span class="token punctuation">.</span>size<span class="token punctuation">)</span> <span class="token operator">==</span> <span class="token number">0</span><span class="token punctuation">;</span></code>',!0),n(o);var p=s(o,6);a(p,()=>`<code class="language-undefined">//Read op as byte, eg: these benchmarks are roughly running at 8 GB/s (not bad).
|               ns/op |                op/s |    err% |     total | benchmark
|--------------------:|--------------------:|--------:|----------:|:----------
|                0.12 |    8,315,064,635.07 |    2.7% |     12.03 | &#96;memcmp&#96;
|                0.11 |    8,936,764,258.15 |    3.2% |     11.83 | &#96;byte cmp&#96;</code>`,!0),n(p);var c=s(p,10);a(c,()=>`<code class="language-undefined">|               ns/op |                op/s |    err% |     total | benchmark
|--------------------:|--------------------:|--------:|----------:|:----------
|                0.10 |    9,643,190,854.87 |    0.0% |      0.00 | &#96;memcmp&#96;
|                0.10 |    9,644,749,316.32 |    0.0% |      0.00 | &#96;byte cmp&#96;</code>`,!0),n(c);var i=s(c,6);a(i,()=>`<code class="language-undefined">|               ns/op |                op/s |    err% |     total | benchmark
|--------------------:|--------------------:|--------:|----------:|:----------
...
|                0.10 |    9,646,511,627.91 |    0.0% |      0.00 | &#96;byte cmp (64)&#96;</code>`,!0),n(i);var r=s(i,10);a(r,()=>`<code class="language-c">constexpr <span class="token keyword">auto</span> <span class="token function">byte_cmp</span><span class="token punctuation">(</span><span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> pattern<span class="token punctuation">,</span> <span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> input<span class="token punctuation">,</span> <span class="token class-name">size_t</span> sz<span class="token punctuation">)</span> noexcept <span class="token punctuation">&#123;</span>
	using register_type <span class="token operator">=</span> std<span class="token operator">::</span><span class="token class-name">conditional_t</span><span class="token operator">&lt;</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token operator">*</span><span class="token punctuation">)</span> <span class="token operator">==</span> <span class="token number">8</span><span class="token punctuation">,</span> <span class="token class-name">uint64_t</span><span class="token punctuation">,</span> <span class="token class-name">uint32_t</span><span class="token operator">></span><span class="token punctuation">;</span>
	using result_type <span class="token operator">=</span> std<span class="token operator">::</span><span class="token class-name">conditional_t</span><span class="token operator">&lt;</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token operator">*</span><span class="token punctuation">)</span> <span class="token operator">==</span> <span class="token number">8</span><span class="token punctuation">,</span> <span class="token class-name">int64_t</span><span class="token punctuation">,</span> <span class="token class-name">int32_t</span><span class="token operator">></span><span class="token punctuation">;</span>
	<span class="token comment">//div rem can be one op</span>
	<span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_count <span class="token operator">=</span> sz <span class="token operator">/</span> <span class="token punctuation">(</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	<span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_rem <span class="token operator">=</span> sz <span class="token operator">%</span> <span class="token punctuation">(</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	<span class="token comment">//do a running sub</span>
	result_type ret <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span>
	<span class="token keyword">for</span> <span class="token punctuation">(</span><span class="token class-name">size_t</span> i <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span> i <span class="token operator">&lt;</span> loop_count<span class="token punctuation">;</span> i<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
		result_type tmp <span class="token operator">=</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> result_type<span class="token operator">*</span><span class="token punctuation">)</span>pattern<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span> <span class="token operator">-</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> result_type<span class="token operator">*</span><span class="token punctuation">)</span>input<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span><span class="token punctuation">;</span>
	...
	pattern <span class="token operator">+=</span> <span class="token punctuation">(</span>loop_count <span class="token operator">*</span> <span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	input <span class="token operator">+=</span> <span class="token punctuation">(</span>loop_count <span class="token operator">*</span> <span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	<span class="token keyword">for</span> <span class="token punctuation">(</span><span class="token class-name">size_t</span> i <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span> i <span class="token operator">&lt;</span> loop_rem<span class="token punctuation">;</span> i<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
		result_type tmp <span class="token operator">=</span> pattern<span class="token punctuation">[</span>i<span class="token punctuation">]</span> <span class="token operator">-</span> input<span class="token punctuation">[</span>i<span class="token punctuation">]</span><span class="token punctuation">;</span>
		<span class="token keyword">if</span> <span class="token punctuation">(</span>tmp <span class="token operator">!=</span> <span class="token number">0</span><span class="token punctuation">)</span>
			<span class="token keyword">return</span> ret <span class="token operator">=</span> tmp<span class="token punctuation">;</span>
	<span class="token punctuation">&#125;</span>

	<span class="token keyword">return</span> ret<span class="token punctuation">;</span>
<span class="token punctuation">&#125;</span></code>`,!0),n(r);var l=s(r,8);a(l,()=>`<code class="language-undefined">|               ns/op |                op/s |    err% |     total | benchmark
|--------------------:|--------------------:|--------:|----------:|:----------
...
|                0.12 |    8,624,424,732.14 |    2.4% |     12.25 | &#96;byte cmp (64)&#96;</code>`,!0),n(l);var u=s(l,12);a(u,()=>`<code class="language-c">    bench<span class="token punctuation">.</span><span class="token function">run</span><span class="token punctuation">(</span><span class="token string">"memcmp"</span><span class="token punctuation">,</span> <span class="token punctuation">[</span><span class="token operator">&amp;</span>count<span class="token punctuation">]</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
        constexpr str_view test <span class="token operator">=</span> <span class="token punctuation">&#123;</span> <span class="token string">"0123456789abcdefA"</span> <span class="token punctuation">&#125;</span><span class="token punctuation">;</span>
        bool result <span class="token operator">=</span> <span class="token punctuation">(</span>test<span class="token punctuation">.</span>size <span class="token operator">>=</span> test<span class="token punctuation">.</span>size<span class="token punctuation">)</span> <span class="token operator">&amp;&amp;</span> std<span class="token operator">::</span><span class="token function">memcmp</span><span class="token punctuation">(</span>test<span class="token punctuation">.</span>ptr<span class="token punctuation">,</span> test<span class="token punctuation">.</span>ptr<span class="token punctuation">,</span> test<span class="token punctuation">.</span>size<span class="token punctuation">)</span> <span class="token operator">==</span> <span class="token number">0</span><span class="token punctuation">;</span>
        count <span class="token operator">+=</span> result<span class="token punctuation">;</span>
    <span class="token punctuation">&#125;</span><span class="token punctuation">)</span><span class="token punctuation">;</span></code>`,!0),n(u);var k=s(u,6);a(k,()=>`<code class="language-undefined">|               ns/op |                op/s |    err% |     total | benchmark
|--------------------:|--------------------:|--------:|----------:|:----------
...
|                0.15 |    6,867,877,477.58 |    4.3% |     11.93 | &#96;byte cmp&#96;</code>`,!0),n(k);var m=s(k,8);a(m,()=>`<code class="language-undefined">|               ns/op |                op/s |    err% |     total | benchmark
|--------------------:|--------------------:|--------:|----------:|:----------
...
|                0.13 |    7,793,473,898.92 |    0.6% |     11.86 | &#96;byte cmp&#96;</code>`,!0),n(m);var h=s(m,10);a(h,()=>`<code class="language-c">constexpr <span class="token keyword">auto</span> <span class="token function">byte_eq</span><span class="token punctuation">(</span><span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> pattern<span class="token punctuation">,</span> <span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> input<span class="token punctuation">,</span> <span class="token class-name">size_t</span> sz<span class="token punctuation">)</span> noexcept <span class="token punctuation">&#123;</span>
	using register_type <span class="token operator">=</span> std<span class="token operator">::</span><span class="token class-name">conditional_t</span><span class="token operator">&lt;</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token operator">*</span><span class="token punctuation">)</span> <span class="token operator">==</span> <span class="token number">8</span><span class="token punctuation">,</span> <span class="token class-name">uint64_t</span><span class="token punctuation">,</span> <span class="token class-name">uint32_t</span><span class="token operator">></span><span class="token punctuation">;</span>
	<span class="token comment">//div rem can be one op</span>
	<span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_count <span class="token operator">=</span> sz <span class="token operator">/</span> <span class="token punctuation">(</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	<span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_rem <span class="token operator">=</span> sz <span class="token operator">%</span> <span class="token punctuation">(</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	<span class="token comment">//this performs oddly</span>
	register_type ret <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span>
	<span class="token keyword">for</span> <span class="token punctuation">(</span>std<span class="token operator">::</span><span class="token class-name">size_t</span> i <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span> i <span class="token operator">&lt;</span> loop_count<span class="token punctuation">;</span> i<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
		ret <span class="token operator">|=</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> register_type<span class="token operator">*</span><span class="token punctuation">)</span>pattern<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span> <span class="token operator">^</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> register_type<span class="token operator">*</span><span class="token punctuation">)</span>input<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span><span class="token punctuation">;</span>
	<span class="token punctuation">&#125;</span>
...
<span class="token punctuation">&#125;</span></code>`,!0),n(h);var y=s(h,4);a(y,()=>`<code class="language-undefined">|               ns/op |                op/s |    err% |     total | benchmark
|--------------------:|--------------------:|--------:|----------:|:----------
|                0.25 |    3,956,294,752.61 |    8.5% |     11.71 | :wavy_dash: &#96;memcmp&#96; (Unstable with ~248,258,903.5 iters. Increase &#96;minEpochIterations&#96; to e.g. 2482589035)
|                0.13 |    7,793,473,898.92 |    0.6% |     11.86 | &#96;byte cmp&#96;
|                0.45 |    2,239,708,217.48 |    3.5% |     11.52 | &#96;byte eq&#96;</code>`,!0),n(y);var b=s(y,6);a(b,()=>`<code class="language-c">constexpr <span class="token keyword">auto</span> <span class="token function">byte_eq_2</span><span class="token punctuation">(</span><span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> pattern<span class="token punctuation">,</span> <span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> input<span class="token punctuation">,</span> <span class="token class-name">size_t</span> sz<span class="token punctuation">)</span> noexcept <span class="token punctuation">&#123;</span>
	using register_type <span class="token operator">=</span> std<span class="token operator">::</span><span class="token class-name">conditional_t</span><span class="token operator">&lt;</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token operator">*</span><span class="token punctuation">)</span> <span class="token operator">==</span> <span class="token number">8</span><span class="token punctuation">,</span> <span class="token class-name">uint64_t</span><span class="token punctuation">,</span> <span class="token class-name">uint32_t</span><span class="token operator">></span><span class="token punctuation">;</span>
	<span class="token comment">//div rem can be one op</span>
	<span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_count <span class="token operator">=</span> sz <span class="token operator">/</span> <span class="token punctuation">(</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	<span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_rem <span class="token operator">=</span> sz <span class="token operator">%</span> <span class="token punctuation">(</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	...
	<span class="token keyword">return</span> ret <span class="token operator">==</span> <span class="token number">0</span><span class="token punctuation">;</span>
<span class="token punctuation">&#125;</span></code>`,!0),n(b);var w=s(b,2);a(w,()=>`<code class="language-undefined">|               ns/op |                op/s |    err% |     total | benchmark
|--------------------:|--------------------:|--------:|----------:|:----------
...
|                0.25 |    3,999,846,415.60 |    5.4% |     11.52 | :wavy_dash: &#96;byte eq (!=)&#96; (Unstable with ~246,409,890.3 iters. Increase &#96;minEpochIterations&#96; to e.g. 2464098903)</code>`,!0),n(w);var d=s(w,10);a(d,()=>`<code class="language-undefined">|               ns/op |                op/s |    err% |     total | benchmark
|--------------------:|--------------------:|--------:|----------:|:----------
...
|                4.36 |      229,466,813.08 |    6.7% |     11.91 | :wavy_dash: &#96;byte eq (!=)&#96; (Unstable with ~14,681,869.8 iters. Increase &#96;minEpochIterations&#96; to e.g. 146818698)</code>`,!0),n(d);var q=s(d,4);a(q,()=>`<code class="language-undefined">|               ns/op |                op/s |    err% |     total | benchmark
|--------------------:|--------------------:|--------:|----------:|:----------
...
|                4.42 |      226,116,576.86 |    5.7% |     11.90 | :wavy_dash: &#96;byte eq (!=)&#96; (Unstable with ~14,237,177.3 iters. Increase &#96;minEpochIterations&#96; to e.g. 142371773)</code>`,!0),n(q);var g=s(q,12);a(g,()=>`<code class="language-c">constexpr <span class="token keyword">auto</span> <span class="token function">byte_eq4</span><span class="token punctuation">(</span><span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> pattern<span class="token punctuation">,</span> <span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> input<span class="token punctuation">,</span> <span class="token class-name">size_t</span> sz<span class="token punctuation">)</span> noexcept <span class="token punctuation">&#123;</span>
	using register_type <span class="token operator">=</span> std<span class="token operator">::</span><span class="token class-name">conditional_t</span><span class="token operator">&lt;</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token operator">*</span><span class="token punctuation">)</span> <span class="token operator">==</span> <span class="token number">8</span><span class="token punctuation">,</span> <span class="token class-name">uint64_t</span><span class="token punctuation">,</span> <span class="token class-name">uint32_t</span><span class="token operator">></span><span class="token punctuation">;</span>
	<span class="token comment">//div rem can be one op</span>
	<span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_count <span class="token operator">=</span> sz <span class="token operator">/</span> <span class="token punctuation">(</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	<span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_rem <span class="token operator">=</span> sz <span class="token operator">%</span> <span class="token punctuation">(</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	...
	<span class="token keyword">for</span> <span class="token punctuation">(</span><span class="token punctuation">;</span> <span class="token punctuation">(</span>i<span class="token operator">+</span><span class="token number">3</span><span class="token punctuation">)</span> <span class="token operator">&lt;</span> loop_count<span class="token punctuation">;</span> i<span class="token operator">+=</span><span class="token number">4</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
		ret <span class="token operator">|=</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> register_type<span class="token operator">*</span><span class="token punctuation">)</span>pattern<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span> <span class="token operator">^</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> register_type<span class="token operator">*</span><span class="token punctuation">)</span>input<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span><span class="token punctuation">;</span>
		ret <span class="token operator">|=</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> register_type<span class="token operator">*</span><span class="token punctuation">)</span>pattern<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token operator">+</span><span class="token number">1</span><span class="token punctuation">]</span> <span class="token operator">^</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> register_type<span class="token operator">*</span><span class="token punctuation">)</span>input<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token operator">+</span><span class="token number">1</span><span class="token punctuation">]</span><span class="token punctuation">;</span>
		ret <span class="token operator">|=</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> register_type<span class="token operator">*</span><span class="token punctuation">)</span>pattern<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token operator">+</span><span class="token number">2</span><span class="token punctuation">]</span> <span class="token operator">^</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> register_type<span class="token operator">*</span><span class="token punctuation">)</span>input<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token operator">+</span><span class="token number">2</span><span class="token punctuation">]</span><span class="token punctuation">;</span>
		ret <span class="token operator">|=</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> register_type<span class="token operator">*</span><span class="token punctuation">)</span>pattern<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token operator">+</span><span class="token number">3</span><span class="token punctuation">]</span> <span class="token operator">^</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> register_type<span class="token operator">*</span><span class="token punctuation">)</span>input<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token operator">+</span><span class="token number">3</span><span class="token punctuation">]</span><span class="token punctuation">;</span>
	<span class="token punctuation">&#125;</span>
	<span class="token keyword">for</span> <span class="token punctuation">(</span><span class="token punctuation">;</span> i <span class="token operator">&lt;</span> loop_count<span class="token punctuation">;</span> i<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
		ret <span class="token operator">|=</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> register_type<span class="token operator">*</span><span class="token punctuation">)</span>pattern<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span> <span class="token operator">^</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> register_type<span class="token operator">*</span><span class="token punctuation">)</span>input<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span><span class="token punctuation">;</span>
	<span class="token punctuation">&#125;</span>

	pattern <span class="token operator">+=</span> <span class="token punctuation">(</span>loop_count <span class="token operator">*</span> <span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	input <span class="token operator">+=</span> <span class="token punctuation">(</span>loop_count <span class="token operator">*</span> <span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	<span class="token keyword">for</span> <span class="token punctuation">(</span><span class="token punctuation">;</span> i <span class="token operator">&lt;</span> sz<span class="token punctuation">;</span> i<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
		ret <span class="token operator">|=</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span><span class="token punctuation">)</span>pattern<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span> <span class="token operator">^</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span><span class="token punctuation">)</span>input<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span><span class="token punctuation">;</span>
	<span class="token punctuation">&#125;</span>
...
constexpr <span class="token keyword">auto</span> <span class="token function">byte_eq5</span><span class="token punctuation">(</span><span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> pattern<span class="token punctuation">,</span> <span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> input<span class="token punctuation">,</span> <span class="token class-name">size_t</span> sz<span class="token punctuation">)</span> noexcept <span class="token punctuation">&#123;</span>
	using register_type <span class="token operator">=</span> std<span class="token operator">::</span><span class="token class-name">conditional_t</span><span class="token operator">&lt;</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token operator">*</span><span class="token punctuation">)</span> <span class="token operator">==</span> <span class="token number">8</span><span class="token punctuation">,</span> <span class="token class-name">uint64_t</span><span class="token punctuation">,</span> <span class="token class-name">uint32_t</span><span class="token operator">></span><span class="token punctuation">;</span>
	<span class="token comment">//div rem can be one op</span>
	<span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_count <span class="token operator">=</span> sz <span class="token operator">/</span> <span class="token punctuation">(</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	<span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_rem <span class="token operator">=</span> sz <span class="token operator">%</span> <span class="token punctuation">(</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span>register_type<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	...
	<span class="token keyword">return</span> ret<span class="token punctuation">;</span>
<span class="token punctuation">&#125;</span></code>`,!0),n(g);var I=s(g,2);a(I,()=>`<code class="language-undefined">|               ns/op |                op/s |    err% |     total | benchmark
|--------------------:|--------------------:|--------:|----------:|:----------
...
|                0.66 |    1,524,836,223.99 |    8.6% |     11.99 | :wavy_dash: &#96;byte eq (unroll branch)&#96; (Unstable with ~98,337,616.8 iters. Increase &#96;minEpochIterations&#96; to e.g. 983376168)</code>`,!0),n(I);var _=s(I,6);a(_,()=>`<code class="language-c"><span class="token keyword">int</span> <span class="token function">byte_eq_sse_negated</span><span class="token punctuation">(</span><span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> pattern<span class="token punctuation">,</span> <span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span> input<span class="token punctuation">,</span> <span class="token class-name">size_t</span> sz<span class="token punctuation">)</span> noexcept <span class="token punctuation">&#123;</span>
	<span class="token class-name">int32_t</span> ret <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span>
	<span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_count <span class="token operator">=</span> sz <span class="token operator">/</span> <span class="token punctuation">(</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span>__m128i<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	<span class="token keyword">const</span> <span class="token class-name">size_t</span> loop_rem <span class="token operator">=</span> sz <span class="token operator">%</span> <span class="token punctuation">(</span><span class="token keyword">sizeof</span><span class="token punctuation">(</span>__m128i<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>

	<span class="token keyword">for</span> <span class="token punctuation">(</span><span class="token class-name">size_t</span> i <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span> i <span class="token operator">&lt;</span> loop_count<span class="token punctuation">;</span> i<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
		__m128i lhs <span class="token operator">=</span> <span class="token function">_mm_loadu_si128</span><span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> __m128i<span class="token operator">*</span><span class="token punctuation">)</span>pattern<span class="token punctuation">)</span> <span class="token operator">+</span> i<span class="token punctuation">)</span><span class="token punctuation">;</span>
		__m128i rhs <span class="token operator">=</span> <span class="token function">_mm_loadu_si128</span><span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> __m128i<span class="token operator">*</span><span class="token punctuation">)</span>input<span class="token punctuation">)</span> <span class="token operator">+</span> i<span class="token punctuation">)</span><span class="token punctuation">;</span>
		__m128i tmp <span class="token operator">=</span> <span class="token function">_mm_cmpeq_epi8</span><span class="token punctuation">(</span>lhs<span class="token punctuation">,</span> rhs<span class="token punctuation">)</span><span class="token punctuation">;</span>
		ret <span class="token operator">|=</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token operator">~</span><span class="token function">_mm_movemask_epi8</span><span class="token punctuation">(</span>tmp<span class="token punctuation">)</span><span class="token punctuation">)</span> <span class="token operator">&amp;</span> <span class="token number">0xffff</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
		<span class="token keyword">if</span> <span class="token punctuation">(</span>ret <span class="token operator">!=</span> <span class="token number">0</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
			<span class="token keyword">return</span> ret<span class="token punctuation">;</span>
		<span class="token punctuation">&#125;</span>
	<span class="token punctuation">&#125;</span>

	pattern <span class="token operator">+=</span> <span class="token punctuation">(</span>loop_count <span class="token operator">*</span> <span class="token keyword">sizeof</span><span class="token punctuation">(</span>__m128i<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	input <span class="token operator">+=</span> <span class="token punctuation">(</span>loop_count <span class="token operator">*</span> <span class="token keyword">sizeof</span><span class="token punctuation">(</span>__m128i<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
	<span class="token keyword">for</span> <span class="token punctuation">(</span><span class="token class-name">size_t</span> i <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span> i <span class="token operator">&lt;</span> loop_rem<span class="token punctuation">;</span> i<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
		ret <span class="token operator">|=</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span><span class="token punctuation">)</span>pattern<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span> <span class="token operator">^</span> <span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token keyword">const</span> <span class="token keyword">char</span><span class="token operator">*</span><span class="token punctuation">)</span>input<span class="token punctuation">)</span><span class="token punctuation">[</span>i<span class="token punctuation">]</span><span class="token punctuation">;</span>
	<span class="token punctuation">&#125;</span>
	<span class="token keyword">return</span> ret<span class="token punctuation">;</span>
<span class="token punctuation">&#125;</span></code>`,!0),n(_);var f=s(_,2);a(f,()=>`<code class="language-undefined">
|               ns/op |                op/s |    err% |     total | benchmark
|--------------------:|--------------------:|--------:|----------:|:----------
...
|                0.64 |    1,570,911,526.41 |    2.3% |     12.18 | &#96;byte eq (sse negated)&#96;</code>`,!0),n(f),A(10),E(U,v)}export{R as default,W as metadata};
